...
<div class="max-w-2xl mx-auto">
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-2xl font-semibold">Edit Budget</h2>
  ...
  <div class="bg-white shadow rounded-lg p-6">
    <form method="POST" action="{{ route('budgets.update', $budget) }}">
      ...
      <input type="number" step="0.01" name="amount" value="{{ old('amount', $budget->amount) }}" class="mt-1 block w-full border rounded px-3 py-2 text-sm" required>

      <div class="grid grid-cols-2 gap-4 mt-4">
        <div>
          <label class="block text-sm font-medium text-gray-700">Start date</label>
          <input type="date" name="start_date" value="{{ old('start_date', $budget->start_date) }}" class="mt-1 block w-full border rounded px-3 py-2 text-sm" required>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">End date</label>
          <input type="date" name="end_date" value="{{ old('end_date', $budget->end_date) }}" class="mt-1 block w-full border rounded px-3 py-2 text-sm" required>
        </div>
      </div>
